...
import com.example.projectmanageweb.dto.ForgotPasswordRequest;
...
public class ForgotPasswordController {
    ...
    @PostMapping("/request")
    public String sendOtp(@Valid @ModelAttribute("form") ForgotPasswordRequest form,
        ...
        try {
            boolean success = otpService.sendOtpToEmail(form.getEmail());
            if (success) {
    ...
    @GetMapping("/verify")
    public String verifyOtpForm(Model model) {
        if (!model.containsAttribute("form")) {
            ResetPasswordRequest form = new ResetPasswordRequest();
            // Lấy email từ flash attribute nếu có
            if (model.containsAttribute("email")) {
                form.setEmail((String) model.getAttribute("email"));
            }
    ...
    @PostMapping("/verify")
    public String resetPassword(@Valid @ModelAttribute("form") ResetPasswordRequest form,
                               BindingResult bindingResult,
                               RedirectAttributes redirectAttributes,
                               Model model) {
        ...
        try {
            // Xác thực OTP
            boolean otpValid = otpService.verifyOtp(form.getEmail(), form.getOtpCode());
            if (!otpValid) {
